...
async def prepare_prediction_data(branch_id: str) -> pd.DataFrame:
    ...
    for item in items_list:
        ...
        if not prices_list or len(prices_list) < SEQUENCE_LENGTH:
            print(
                f"  Skipping {item['name']}: insufficient history ({len(prices_list)} records)"
            )
            continue
...
async def predict_prices(branch_id: str) -> dict:
    try:
        ...
        if not model_path.exists():
            return returnFormat(
                "error", f"No model found for branch {branch_id}. Train a model first."
            )

        if not preprocessor_path.exists():
            return returnFormat(
                "error", f"No preprocessor found for branch {branch_id}."
            )
        ...
        for item_id in transformed["item_id"].unique():
            item_data = transformed[transformed["item_id"] == item_id].sort_values(
                "timestamp"
            )
            ...
            predicted_change = np.clip(
                predicted_change, -MAX_PRICE_CHANGE * 100, MAX_PRICE_CHANGE * 100
            )
            ...
            recommendation = generate_recommendation(
                current_price, predicted_change, confidence, demand_category
            )
        ...
        normal_demand = len(
            [p for p in predictions if p["demand_category"] == "NORMAL"]
        )
